...
function renderMarketing(m, uid, wrapClass, enrichment, sfVal) {
  if (!m && !enrichment) return "";
  const safe = s => esc(s||"");
  ...
  if (enrichment) {
    const e = enrichment;
    const sfStr = sfVal ? sfVal + " SF available" : "Space available";
    const tenantLines = (e.ideal_tenants || []).slice(0, 4).map(t => {
      const brands = (t.example_brands || []).slice(0, 2).join(", ");
      return "• " + t.category + (brands ? " — " + brands : "");
    }).join("\\n");
    ...
    const creBody = [
      ...
      "",
      sfStr,
      e.leasing_angle ? "Leasing angle: " + e.leasing_angle : "",
    ...
    addTab("costar",  "CoStar/LoopNet", post(creBody));
    addTab("crexi",   "Crexi",          post(creBody));
    addTab("zillow",  "Zillow",         post(consumerBody));
    addTab("redfin",  "Redfin",         post(consumerBody));
    addTab("realtor", "Realtor.com",    post(consumerBody));
  }

  if (!tabs.length) return "";
  const ctx = m && m.local_context ? `<div class="local-context">${safe(m.local_context)}</div>` : "";

  if (wrap === "modal") {
    return `<div class="modal-marketing">${ctx}<div class="channel-tabs">${tabs.join("")}</div>${panels.join("")}</div>`;
  }
  return `<details class="${wrap}"><summary>Marketing &amp; Listing Content (${tabs.length})</summary>${ctx}<div class="channel-tabs">${tabs.join("")}</div>${panels.join("")}</details>`;
}
...
function render() {
  ...
  grid.innerHTML = filtered.map(l => {
    ...
    return `<article class="card">
      ...
      ${description}${angle}${tenants}${renderMarketing(l.m, uid, "", l.e, l.sf)}
...
window.openModal = function(key) {
  ...
  cardCounter = 1000;
  document.getElementById("modal-content").innerHTML = renderMarketing(listing.m, cardId(), "modal", listing.e, p.sf);
